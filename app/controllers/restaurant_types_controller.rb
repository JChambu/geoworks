class RestaurantTypesController < ApplicationController
  ...
  def index
    @restaurant_types = RestaurantType.sorted_by_name.paginate(:page=> params[:page])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @restaurant_type.save
        ...
        format.json { render action: 'show', status: :created, location: @restaurant_type }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to restaurant_types_url }
      format.json { head :no_content }
    end
  end
